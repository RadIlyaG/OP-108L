...
# SQliteAddLine
# ***************************************************************************
proc SQliteAddLine {unit} {
  global gaSet
  set unitNum [string index $unit end]
  set barcode $gaSet($gaSet(pair).barcode$unitNum)
  puts "[MyTime] SQliteAddLine $barcode"
  if {[string match *skip* $barcode]} {
    ## do not include skipped in stats
    return 0
  }
  if {$gaSet($gaSet(pair).barcode$unitNum.IdMacLink)=="link"} {
    ## do not report about passed unit
    # 10:39 04/10/2023 return 0
  }
